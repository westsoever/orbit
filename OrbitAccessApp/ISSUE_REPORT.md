...
- [x] **Status bar glyphs:** `○` idle · `●` capturing · `◐` browse-only · `×` no database
...
- [x] **Offline-first mode:** browse saved context, lexical search, offline snippet chat, and read-only pending tasks without daemon; AI chat/hybrid search/dispatch require daemon
- [x] **Keychain password prompt:** removed Keychain-stored security-scoped bookmarks; app reads `~/.orbit/orbit.db` directly via sandbox home-path entitlement (no repeated login prompts on rebuild)
